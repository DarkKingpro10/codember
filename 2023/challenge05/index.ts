console.time("Challenge 05");

const LOGS = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
var invalidUserNameLogs: string[] = [];

function isAlphaNumeric(word: string) {
...
LOGS.split("\n").forEach((log) => {
	const [id, username, email, age, city] = log.split(",");

	const addInvalidUser = () => {
		invalidUserNameLogs.push(username[0]);
	};

	if (!id || !isAlphaNumeric(id)) {
		addInvalidUser();
		return;
	} else if (!username || !isAlphaNumeric(username)) {
		addInvalidUser();
		return;
...
console.log(invalidUserNameLogs.join(""));

console.timeEnd("Challenge 05");
