 ...
 */
  ...
  const answer: string = MESSAGE.toLowerCase().split(" ").reduce((acc: params, word: string)=>{
    const index = acc.indexOf(word);

    if (acc.includes(word) && typeof acc[index + 1] === "number") {
      acc[index + 1] = (acc[index + 1] as number) + 1;
    } else {
      acc.push(word, 1);
    }
    return acc;
