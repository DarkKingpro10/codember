...
POLICY_PASSWORDS.split('\n').forEach(policyPassword => {
  let rule: string = policyPassword.split(':')[0];
  let password: string = policyPassword.split(':')[1];
  let letter: string = rule.split(' ')[1];
  let min: string = rule.split(' ')[0].split('-')[0];
  let max: string = rule.split(' ')[0].split('-')[0];
