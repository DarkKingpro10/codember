console.time('Challenge 03');

const POLICY_PASSWORDS = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
POLICY_PASSWORDS.split('\n').forEach(policyPassword => {
  let rule = policyPassword.split(':')[0];
  let password = policyPassword.split(':')[1].trim().toLowerCase();
  let letter = rule.split(' ')[1];
  let min = parseInt(rule.split(' ')[0].split('-')[0]);
  let max = parseInt(rule.split(' ')[0].split('-')[1]);

  let regex = new RegExp(`^(?=(?:[^${letter}]*${letter}){${min},})(?!(?:[^${letter}]*${letter}){${max+1}}).*$`);
  
  if(!regex.test(password)){
    invalidKeys.push(password);
  }
