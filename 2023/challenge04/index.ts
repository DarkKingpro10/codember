...
FILES.split("\n").forEach(file =>{
  ...
  let checkSum: string = file.split('-')[1];
  
  let uniqueLetters: string[] = []

  fileName.split("").forEach((letter: string)=>{
    let regex: RegExp = new RegExp(`^(?!.*${letter}.*${letter}).*${letter}.*$`);
  ...
  if (uniqueLetters.join("") === checkSum) {
    realFiles = [...realFiles, [fileName, checkSum]];
  }
